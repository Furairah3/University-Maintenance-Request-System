...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Auth::validateCSRF($_POST['csrf_token'] ?? '')) {
        ...
        elseif ($action === 'update_prefs') {
            ...
            $db->execute("UPDATE users SET email_notifications = ? WHERE id = ?", [$emailNotif, $userId]);
...
<div style="display:flex;flex-direction:column;gap:20px;max-width:700px;">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <?php if ($photoError): ?><div class="alert alert-error"><?= htmlspecialchars($photoError) ?></div><?php endif; ?>
            <?php if ($photoSuccess): ?><div class="alert alert-success"><?= htmlspecialchars($photoSuccess) ?></div><?php endif; ?>

            <div style="display:flex;gap:20px;align-items:center;flex-wrap:wrap;">
                <div style="width:96px;height:96px;border-radius:50%;overflow:hidden;background:#dc2626;display:flex;align-items:center;justify-content:center;color:#fff;font-size:36px;font-weight:600;flex-shrink:0;">
                    <?php if (!empty($user['profile_image'])): ?>
                        <img src="<?= APP_URL . '/' . htmlspecialchars($user['profile_image']) ?>?v=<?= time() ?>" alt="Profile picture" style="width:100%;height:100%;object-fit:cover;">
                    <?php else: ?>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="form-row mb-2">
                <div><div class="text-muted" style="font-size:12px;">Email</div><div><?= htmlspecialchars($user['email']) ?></div></div>
                <div><div class="text-muted" style="font-size:12px;">Role</div><div>Administrator</div></div>
            </div>
            <div class="form-row">
                <div><div class="text-muted" style="font-size:12px;">Member since</div><div><?= formatDate($user['created_at']) ?></div></div>
                <div><div class="text-muted" style="font-size:12px;">Last login</div><div><?= $user['last_login'] ? formatDate($user['last_login'], 'M d, Y H:i') : '—' ?></div></div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="<?= htmlspecialchars($user['name']) ?>" required minlength="2" maxlength="100">
                </div>
                <div class="form-group">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <?php if ($prefsError): ?><div class="alert alert-error"><?= htmlspecialchars($prefsError) ?></div><?php endif; ?>
            <?php if ($prefsSuccess): ?><div class="alert alert-success"><?= htmlspecialchars($prefsSuccess) ?></div><?php endif; ?>
            <form method="POST" action="">
                <?= csrfField() ?>
                <input type="hidden" name="action" value="update_prefs">
                <div class="form-group">
                    <label style="display:flex;gap:10px;align-items:center;cursor:pointer;">
                        <input type="checkbox" name="email_notifications" value="1" <?= $user['email_notifications'] ? 'checked' : '' ?> style="width:18px;height:18px;">
                        <span>
                            <strong>Email notifications</strong><br>
                            <small class="text-muted" style="font-size:12px;">Receive email updates on significant system events.</small>
                        </span>
                    </label>
                </div>
